...
class ModelTraining:
    ...
    def save_metrics_to_csv(self, state: TrainingState, output_dir: str) -> None:
        ...
        for model_name, metrics in state.model_metrics.items():
            ...
            params_data.append({
                'Model': model_name,
                'Best_Parameters': json.dumps(best_params, indent=2),
                'CV_Score': round(metrics.get('best_cv_score', 0), 4)
            })
        ...
        best_model_info = {
            ...
            'Value': [
                ...
                round(state.model_metrics[state.best_model_name].get('best_cv_score', 0), 4),
    ...
    def train_models(self, state: TrainingState) -> TrainingState:
        ...
        try:
            ...
            logger.info(f"Time: {time.time() - stack_start:.2f}s | "
