...
def _bar_chart_svg(
    ...
    for i, d in enumerate(data):
        ...
        labels.append(
            ...
            f'text-anchor="middle" font-size="10" fill="#666">{escape(str(d.get(label_key, "")))}</text>'
        )

    return f"""
    <svg width="{width}" height="{height}" xmlns="http://www.w3.org/2000/svg">
        <text x="{width/2}" y="18" text-anchor="middle" font-size="13" font-weight="600" fill="#333">{title}</text>
        {''.join(bars)}
        {''.join(labels)}
    </svg>
    """
    ...
    if include_charts and total > 0:
        ...
        <div class="charts-section">
            <div class="chart-container">
                {_bar_chart_svg(dist_data, "value", "label", "Prediction Distribution", bar_color="#4a90d9", width=400, height=200)}
            </div>
        </div>
        """
    ...
    if include_details and results:
        ...
        for i, r in enumerate(results[:200]):  # Limit to 200 rows for file size
            ...
            if r.get("suspicious_urls", 0) > 0:
                urls_info = f'<span class="badge badge-danger">{r["suspicious_urls"]} susp.</span>'
            elif r.get("url_count", 0) > 0:
                urls_info = f'<span class="badge badge-ok">{r["url_count"]} urls</span>'
...
<body>
    <div class="report-header">
        <h1>📧 {escape(title)}</h1>
        <p>Generated on {datetime.now().strftime("%B %d, %Y at %H:%M")} &middot; {total} emails classified</p>
    </div>

    <div class="summary-cards">
        <div class="card"><div class="value">{total}</div><div class="label">Total Emails</div></div>
        <div class="card spam"><div class="value">{spam_count}</div><div class="label">Spam ({spam_pct}%)</div></div>
    ...
    <div class="report-footer">
        Generated by Spam Email Classifier &middot; Built with scikit-learn &middot; {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
    </div>
</body>
</html>"""
...
def generate_email_report(
    ...
    """
    spam_class = "danger" if prediction == "Spam" else "success"
    risk_color = "#e53935" if prediction == "Spam" else "#43a047"
    ...
    if url_analysis and url_analysis.get("total_urls", 0) > 0:
        ...
        for u in url_analysis.get("urls", []):
            ...
            url_rows.append(f"""
            <tr>
                <td style="word-break:break-all;max-width:300px;">{escape(str(u.get("url", "")))}</td>
                <td>{escape(str(u.get("hostname", "")))}</td>
                <td><span style="background:{risk_bg};color:#fff;padding:1px 8px;border-radius:8px;font-size:0.75rem;">{risk_label}</span></td>
        ...
        <p>Found {url_analysis['total_urls']} URL(s), {url_analysis['suspicious_count']} suspicious.
        Overall URL risk: <strong>{url_analysis['overall_risk_score']:.0f}% ({url_analysis['risk_level'].upper()})</strong></p>
...
<style>
    body {{ font-family: -apple-system, sans-serif; color: #1a1a2e; background: #f8f9fa; padding: 20px; }}
    ...
    .prediction-badge {{ display:inline-block; padding:6px 20px; border-radius:20px; font-weight:700; font-size:1.2rem;
    ...
    .email-content {{ background:#f5f5f5; border-radius:8px; padding:12px; font-size:0.9rem; line-height:1.5;
...
<body>
    <div class="header">
        ...
        <p style="margin-top:8px;opacity:0.85;">
            Confidence: <strong>{f"{confidence:.1f}%" if confidence else "N/A"}</strong>
            &middot; Spam Risk: <strong>{f"{spam_risk:.1f}%" if spam_risk else "N/A"}</strong>
        </p>
    </div>
    ...
    {f'<div class="section"><h3>🧠 Explanation</h3><p>{escape(explanation_summary)}</p></div>' if explanation_summary else ""}

    <div class="footer">
        Generated by Spam Email Classifier &middot; {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
    </div>
</body>
</html>"""
