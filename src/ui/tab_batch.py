...
class BatchPredictionTab:
    """Class-based facade for the batch-processing tab (test-facing API).

    Construction must not touch Streamlit runtime state, so this only
    validates that the module is importable — the functional entry point
    remains :func:`render_batch_processing`.
    """
...
def render_batch_processing(
    ...
    if upload_type == "MBOX / Text":
        st.markdown("Upload an MBOX file (exported from Gmail, Thunderbird, etc.) to classify all emails at once.")
                    ...
                    with st.spinner(f"📊 Classifying {len(data_df)} emails..."):
                        try:
                            ...
                            col2.metric(
                                "Spam",
                                spam_count,
                                delta=f"{spam_count / len(data_df) * 100:.1f}%" if len(data_df) > 0 else "0%",
                                delta_color="inverse",
                            )
                            ...
                            col3.metric(
                                "Ham",
                                ham_count,
                                delta=f"{ham_count / len(data_df) * 100:.1f}%" if len(data_df) > 0 else "0%",
                            )
