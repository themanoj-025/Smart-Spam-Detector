...
def render_history(history_manager: Any) -> None:
    ...
    st.markdown("Browse past classifications, track trends over time, and search through your prediction history.")
    ...
    _page_col1, page_col2, _page_col3 = st.columns([1, 2, 1])

    with page_col2:
        st.caption(f"Page {st.session_state.history_page} of {total_pages} ({total_count} records)")

    nav_col1, _nav_col2, _nav_col3, nav_col4, nav_col5 = st.columns([1, 1, 2, 1, 1])
            ...
            display_data.append(
                {
                    ...
                    "Source": r.get("source", ""),
                    "URLs": (
                        f"{r.get('suspicious_urls', 0)} susp."
                        if r.get("suspicious_urls", 0) > 0
                        else str(r.get("url_count", 0))
                    ),
                    "Subject": (
                        r.get("email_subject", "")[:80] if r.get("email_subject") else r.get("email_text", "")[:80]
                    ),
                }
            )
