...
if _PROM_AVAILABLE:
    ...
    SPAM_PREDICTIONS = Counter("spam_predictions_total", "Email predictions made", ["result"])
...
async def add_security_headers(request: Request, call_next) -> None:
    ...
    if _PROM_AVAILABLE:
        ...
        SPAM_REQUEST_COUNT.labels(method=request.method, endpoint=path, status=response.status_code).inc()
...
async def model_info(request: Request) -> ModelInfo:
    ...
    return ModelInfo(
        ...
        vectorizer_name=(
            os.path.basename(pipeline.config.feature_path) if pipeline and pipeline.config.feature_path else None
        ),
...
async def predict_with_explanation(request: Request, body: PredictRequest) -> PredictResponse:
